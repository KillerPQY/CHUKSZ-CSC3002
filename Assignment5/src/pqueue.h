...
void PriorityQueue<ValueType>::enqueue(ValueType value, double priority) {
    ...
    cp->link = NULL;
    // if the queue is empty, let head = the enqueue
    if (count == 0) {
        head = cp;
        count++;
        return;
    ...
    Cell *old = new Cell;
    for (old = head; old->link != NULL; old = old->link) {
        if (old == head) {
            // the enqueue has the smallest priority
            if (cp->priority < old->priority) {
                cp->link = head;
                head = cp;
                count++;
                return;
            }
            // the enqueue has the second smallest priority
            if (old->link->priority > cp->priority) {
                cp->link = old->link;
                old->link = cp;
                count++;
                return;
            ...
            // according to the priority to insert
            if (cp->priority < old->link->priority) {
                cp->link = old->link;
                old->link = cp;
                count++;
                return;
    ...
    // the enqueue has the biggest priority
    old->link = cp;
    count++;
    return;
}
...
ValueType PriorityQueue<ValueType>::dequeue() {
    //TODO
    // if the queue is empty, can't dequeue
    if (isEmpty())
        error("The Queue is empty.");
    // dequeue, let head = the second element
    Cell *cp = head;
...
ValueType PriorityQueue<ValueType>::peek() const {
    //TODO
    // if the queue is empty, can't do peek()
    if (isEmpty())
        error("The Queue is empty.");
    return head->data;
}
