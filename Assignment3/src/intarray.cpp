...
IntArray::IntArray(int n) {
   ...
   for (int i = 0; i < nElements; i++) {
       // Assign 0 to each element in the IntArray
       array[i] = 0;
   }
}
...
//    get should call error with an appropriate message.
    ...
    if (index < 0 || index >= nElements) {
        error("Index must be in between 0 and nElements-1");
    }
    return array[index];
}
...
void IntArray::deepCopy(const IntArray & src) {
    ...
    // Assign each element in src to each element in array
    for (int i = 0; i < nElements; i++) {
            array[i] = src.array[i];
        }
}
...
IntArray::IntArray(const IntArray & src) {
    //TODO
    // Use deepCopy(src) to implement copy constructor directly
    deepCopy(src);
}

IntArray & IntArray::operator=(const IntArray & src) {
    //TODO
    // If this array already bind with src, return it directly.
    // Otherwise, release its memory and use deepCopy(src) to assign src to it
    if (this != &src) {
        delete[] array;
        deepCopy(src);
    }
